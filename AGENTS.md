...
Repository agent source lives in `skillset.yaml` and `.skillset/`. Root `.skillset/skills` and `.skillset/agents` serve local repository work, while `.skillset/plugins/trails` owns the public Trails plugin. Treat `.claude/skills`, `.agents/skills`, `.claude/agents`, `.codex/agents`, `plugin/`, `plugins/`, `.claude-plugin/marketplace.json`, and their `skillset.lock` files as generated Skillset output. Run `bun run skillset:sync` to render source and `bun run skillset:check` to detect content or mode drift; do not hand-edit managed output or locks.
