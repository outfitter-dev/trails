...
Cursor cloud agents bootstrap through the shared lifecycle scripts, the same way Codex and Claude do: `./scripts/bootstrap.sh cursor [--update]` (provider detected via `CURSOR_AGENT`; repo root resolves from the workspace cwd; treated as a remote checkout, so Bun is pinned strictly and auto-repaired). Teardown is provider-agnostic: `./scripts/bootstrap.sh teardown`. The session-startup update script runs `./scripts/bootstrap.sh cursor --update`, which verifies the pinned Bun and runs a non-frozen `bun install`. Toolchain is preinstalled in the VM snapshot and resolved through the shell profile (`~/.bashrc`/`~/.profile`): Bun (pinned by `.bun-version`, installed at `~/.bun`) and Node via nvm (`default` alias). Standard commands live in the [Commands](#commands) section above; the notes below are only the non-obvious caveats.
