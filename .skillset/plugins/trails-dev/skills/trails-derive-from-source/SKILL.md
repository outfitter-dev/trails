...
1. Name the fact being derived: error class, error category, intent, Result accessor, detour cap, rule metadata, surface rendering, package entrypoint, or schema fact.
...
4. If the owner does not export it, prefer adding a narrow owner export before creating a derivation-local list.
...
- Owner modules in `packages/core/src`, `packages/topography/src`, `packages/warden/src/rules`, or the package that owns the primitive.
...
- Do not treat intentionally local policy deny lists as owner-derived facts unless another consumer needs the same data.
...
- The owner source.
- Each derivation or consumer checked.
