  ...
  test('rewrites a slug reference in depends_on to the numeric id', () => {
    const fm: Frontmatter = {
      depends_on: [
        '9',
        '14',
        'adapter-extraction-and-the-with-packaging-model',
      ],
      slug: 'some-draft',
      status: 'draft',
      title: 'Some Draft',
...
depends_on: [9, 14, 16, 22, adapter-extraction-and-the-with-packaging-model]
---
...
describe('frontmatter round-trip after slug rewrite', () => {
  ...
  test('rewriteSlugRefsInFrontmatter + serialize produces numeric depends_on', () => {
    ...
    expect(serialized).not.toContain('adapter-extraction');
  });
