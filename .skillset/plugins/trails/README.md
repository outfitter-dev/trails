...
Skillset renders two packages from this source:

- `plugin/` is the Claude Code bundle. It includes the skills, the `trail-engineer` agent, rules, and a `SessionStart` hook.
- `plugins/trails/agents/` is an Agent Plugins package containing the portable skills. Install that directory with an Agent Plugins-compatible host using the host's plugin instructions.
...
- `.skillset/plugins/trails/skillset.yaml` owns the plugin compatibility version.
- `packages/core/package.json` owns the Trails framework version targeted by the bundled `trails` skill.
- `.skillset/plugins/trails/skills/trails/SKILL.md` records that framework target. Run `bun run plugin:metadata:sync` after package versioning.
- `.claude-plugin/marketplace.json` and `plugin/` are generated by `bun run skillset:sync`; do not edit them directly.
...
The checker compares the generated `plugin/skills/trails` bundle against standard `$HOME` install locations for shared agent skills, Claude, and Codex. It reports whether each path is a copy, symlink, or missing optional path, then checks file drift, stale vocabulary, and `metadata.trails` drift. It does not update installed skill files; refreshing a local install remains an explicit operator action after reviewing the report.
...
- [Plugin Release Runbook](https://github.com/outfitter-dev/trails/blob/main/docs/releases/plugin-release.md)
