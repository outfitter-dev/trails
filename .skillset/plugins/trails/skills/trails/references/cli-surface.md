...
Export app-owned CLI bindings as a surfaces overlay so the runtime and compiled contract share the same routes:
...
`wf search` is a transparent synonym for `wayfind.search`. Compile reads the exported `trailsOverlays` from the app module. For grouped entries and validation rules, see the [CLI surface guide](https://github.com/outfitter-dev/trails/blob/main/docs/surfaces/cli.md).
...
surface(graph, {
  ...
  resolveInput: (cmd) => { // Custom input resolution per command
    // Fill missing input values from the current CLI context.
  },
});
```
