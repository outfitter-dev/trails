...
- Use `trailhead` for surface-side grouped rendering. Use `schema facet` only as descriptive schema-slice prose. Do not invent a core `Facet` primitive, `facet()`, or adapter-kit `facet` config.
