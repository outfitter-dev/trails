...
metadata:
  skillset:
    ...
    target: codex
    version: 1
    source: .claude/skills/clark
    source-file: .claude/skills/clark/SKILL.md
    source-frontmatter:
      model: opus
      effort: max
...
Load the appropriate skill or reference based on the task. In Claude, load the named skill directly. In Codex, if a named skill is not available through the skill loader, read the matching generated `.agents/skills/<skill-name>/SKILL.md`. For Clark's relative references, read them under `.agents/skills/clark/references/`.

- **Pathfinding** (planning, ADRs, horizon exploration): Stay inline as Clark and load the `clark-pathfinding` playbook. Do not dispatch a separate Clark subagent for pathfinding.
