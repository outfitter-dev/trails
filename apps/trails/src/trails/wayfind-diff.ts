...
const wayfindDiffOutputSchema = z.union([
...
const currentDiffOptions = (
  ...
  if (
    ...
    (input.dir !== undefined || input.trailsDbPath !== undefined)
  ) {
    return Result.err(
      new ValidationError(
        'Configured app diff derives its current artifact from --root-dir and --app. Remove --dir and --trails-db-path so they cannot bypass project selection.',
        {
          context: {
  ...
  return Result.ok({
    ...
    ...(input.dir === undefined ? { rootDir: context.app.rootDir } : {}),
