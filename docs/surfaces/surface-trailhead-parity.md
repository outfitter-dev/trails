...
Trailheads start with MCP because MCP pays the tool-count and schema-context cost first. CLI and HTTP remain peer surfaces, but peer-ness does not mean every surface gets the same rendering at the same time.
...
- HTTP parity should be evaluated as route-group rendering or explicitly rejected as a non-fit.
...
- How do positional args, structured input, and completions render when a group is editorial rather than ID-derived?
...
That shape turns ordinary HTTP routes into a generic RPC envelope. A credible HTTP trailhead would need to be route-group rendering, documentation grouping, or OpenAPI organization rather than a replacement for direct trail routes:
...
- Can a grouped route preserve stable URLs, verbs, request parsing, and error rendering without inventing an HTTP-specific action envelope?
...
- Warden or Topography evidence shows a durable rendering pattern that surfaces can consume without weakening trail identity.
