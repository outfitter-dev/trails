...
Storage stays flat. Each `TraceRecord` is an independent row with explicit lineage:
...
```typescript
await ctx.trace('db-query', async () => {
...
- **`trace` as a verb and noun needs careful API naming.** The ergonomic win is worth the extra review attention.
...
- [ADR-0009: Resources as a First-Class Primitive](0009-first-class-resources.md)
