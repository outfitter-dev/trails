...
The Trails repository will serve as the distribution proof for this migration. Before the implementation cutover is considered complete, its root config must declare all seven runnable apps by canonical topo ID: `trails`, `demo`, `junction`, `lookout`, `packlist`, `stash`, and `switchback`. Every app must satisfy the static binding and own a deterministic lock. The flagship `apps/trails/trails.lock` must become committed evidence rather than a temporary artifact, `apps/trails-demo/trails.lock` must be added after deterministic lock generation lands, and the stable-cutover runbook must stop deleting the flagship lock.
