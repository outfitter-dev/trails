...
- [ADR-0008: Deterministic Surface Derivation](0008-deterministic-trailhead-derivation.md) — the TopoGraph derivation that the topo store persists and queries
...
[^topo-graph]: The TopoGraph is generated by `deriveTopoGraph()` in
    `@ontrails/topographer` and written to `.trails/topo.lock` by
    `writeTopoGraph()`. See `packages/topographer/src/io.ts`.
