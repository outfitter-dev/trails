...
But the surfaces Trails produces are universally consumable. A CLI built with Trails works in any shell. An MCP server built with Trails works with any MCP client. An HTTP surface serves standard HTTP. The runtime is opinionated; the outputs are standard.
...
- 2026-04-17: In-place vocabulary update per ADR-0035 Cutover 3 — title updated to Surface-Agnostic, surface map generation terminology aligned.
