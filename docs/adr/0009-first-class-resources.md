...
Resource `create` factories return `Result`. Thrown exceptions are wrapped as `InternalError` with the service ID in context. A failed resource resolution short-circuits execution with a clear error.
...
export const storageProvision = resource('storage', { /* ... */ });
export const transactionGate = (svc: Resource<Storage>): Layer => ({
  name: 'transaction',
