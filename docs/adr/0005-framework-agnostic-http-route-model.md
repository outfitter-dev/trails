...
- **`execute(input, requestId?, abortSignal?, context?)`** — validates input, composes layers, resolves request auth when configured, runs the blazed trail, returns `Result`
