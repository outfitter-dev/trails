...
| **Layer** | Cross-cutting execution wrapping | `configGate` | `authGate` | `trackerGate` |
...
1. **Config locks first.** It's the foundation — permits and tracker depend on resolved config. Building it pressure-tests the resource + layer + trails trifecta against a real use case.
