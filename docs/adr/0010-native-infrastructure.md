...
### The service + layer + trails trifecta
...
- **Warden governs them.** The same rules apply — no throws, Result returns, follow declarations match usage.
- **Surfaces can filter them.** A surface that wants to hide infrastructure trails filters on `metadata.category`. The default is to expose everything.
...
1. **Bootstrap phase.** Config resolves from static sources — TypeScript config files, environment variables, `.trails/config/` overrides. No trails involved. This produces the resolved config that services need to create.
...
└── surface.lock     — committed
```

Auto-created on first framework operation. The framework generates a `.gitignore` inside `.trails/` that excludes `config/` and `dev/`. `generated/` and `surface.lock` are committed — they're derived artifacts that should be diffable and reviewable.

The workspace gives infrastructure a known home. Config reads overrides from `.trails/config/`. Dev tooling writes state to `.trails/dev/`. Generated surface maps land in `.trails/generated/`. No more scattering framework files across the project root.
...
The built-in for each package is functional enough for development and simple production cases. Adapters are for teams that need specific providers. This follows the logging precedent: `@ontrails/logging` works out of the box, `/logtape` is there when you need it.
...
- **Every infrastructure package follows the same shape.** Service + layer + trails. Developers learn the pattern once. New infrastructure packages follow the template.
...
- **`testAll` validates the entire system in one call.** Infrastructure mocks are on the service definitions. No test harness, no setup files, no per-package configuration. When a service lacks a mock, `testAll` skips trails that depend on it and reports which services blocked which trails — clear guidance, not a silent pass or a cryptic failure.
