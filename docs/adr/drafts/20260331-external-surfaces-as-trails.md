...
Trails defines a contract (the trail) and projects it onto surfaces (CLI, MCP, HTTP). The flow is always inside-out: author a trail, derive the surface. This is the core of the framework and it works.

But the world is full of existing surfaces that developers want to compose with: CLI tools (`git`, `docker`, `kubectl`, `ffmpeg`, `gh`), MCP servers (GitHub, Slack, Linear), and HTTP APIs (REST endpoints, third-party services). These surfaces already exist. They have capabilities. They have implicit or explicit contracts. But they're not trails, so they can't participate in the Trails ecosystem: no typed composition via `follow`, no cross-surface derivation, no `testExamples`, no warden governance, no survey introspection.
...
Trails' core flow: **trail → surface.** Define a contract, derive the projection.

The inverse: **surface → trail.** Observe an existing surface, capture it as a contract.

This is not wrapping. Wrapping hides the external thing behind your code. Rigging captures the external thing's contract in the framework's terms. The external surface's shape (flags, tool schemas, API endpoints), its version, and its observed behavior become tracked, governed, testable artifacts.

### Three external surface types
...
A rigged trail is a trail whose implementation delegates to an external surface. The contract is yours. The capability is theirs. The rigging is the bridge.

## Decision

### `rig()` produces a Trail

`rig()` is a factory function that returns a Trail. Not a new primitive type. A real Trail that composes, tests, surfaces, and governs like any other. The implementation is generated: it delegates to the external surface, maps input to the surface's calling convention, and parses the output into the trail's output schema.
...
The returned object is a `Trail`. `topo('myapp', gitLog)` works. `ctx.follow('git.log', input)` works. `blaze(app)` surfaces it on CLI, MCP, HTTP. `testExamples` tests it (using the mock in CI, using the real binary in integration).
...
The `probe` command discovers the shape of an external surface:
...
The rig lock is stored alongside the rig source code (`.trails/rigs/<name>/rig.lock` or within the pack directory). It records what the external surface looked like at discovery time. Re-probing and diffing detects version drift.
...
Runs the rigged trail against the real external surface. Captures the input and the parsed output as an example. The developer reviews and commits the example.
...
**Integration examples** (captured, environment-dependent) use the real binary. They run in integration test environments. They validate that the parse function still produces correct output for the current version of the external surface.

The distinction is tracked. `testExamples` in standard mode uses mocks. `testExamples --integration` (or `trails rig test --integration`) uses the real external surface.
...
Failing integration examples mean the external surface's behavior changed in ways the parse function doesn't handle. The failures are the update checklist.
...
- **Rig chain depth.** A trail follows a rigged trail that follows another rigged trail. Each hop adds latency and failure surface. Informational.
...
No dependency on surface packages. Rig produces trails. What happens to those trails (CLI, MCP, HTTP surfaces) is the topo's concern.
...
This follows ADR-0000: Bun-native, universally consumable. The rig package is Bun-native. The trails it produces are universally consumable on any surface.
...
Rigged trails can emit events and declare triggers like any other trail:

```typescript
const gitStatus = rig('git.status', {
  source: { type: 'cli', command: 'git status', binary: 'git' },
  intent: 'read',
  on: trigger('schedule', { cron: '*/5 * * * *' }),
  emits: [uncommittedChangesDetected],
  input: z.object({}),
  ...
  run: async (input, ctx) => {
    ...
    if (status.isOk() && status.value.length > 0) {
      ctx.emit(uncommittedChangesDetected, { files: status.value });
    }
    return status;
  },
});
...
A rigged trail with a schedule trigger becomes a periodic monitoring probe. The rig captures the command output. The trigger activates it on a schedule. The parse function structures the output. `ctx.emit()` announces what was observed. Other trails trigger on the event. Crumbs records every execution.
...
- **Existing surfaces become composable.** CLI tools, MCP servers, and HTTP APIs participate in `follow` chains, `testExamples`, survey, and warden governance. The entire Trails ecosystem opens to external capability.
- **No new primitive type.** `rig()` returns a Trail. The topo, surfaces, testing, and governance don't need to learn a new concept. A rigged trail is a trail.
- **Version drift is tracked.** Rig locks record the external surface's shape at a point in time. Probes detect drift. Integration examples verify behavior. CI catches staleness. External dependencies become governed artifacts.
- **The contract is the firewall.** When the external surface changes, the rig's parse function and examples absorb the change. Consumers of the rigged trail see a stable contract. The rig isolates the instability.
- **Zero external dependencies.** The entire rig implementation (probe, execute, capture, check) uses Bun-native APIs: `Bun.which()`, `Bun.spawn()`, `Bun.$`, `Bun.Terminal`, `Bun.stripANSI()`. On Node the same functionality would require six or more npm packages. This keeps the supply chain surface minimal.
