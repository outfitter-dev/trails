...
1. **Config composition implies boundaries.** The config ADR's env prefix scoping behaves like a namespace boundary — a pack's config schema gets prefixed when composed into a topo. That pattern implies a boundary that doesn't yet exist at the trail/resource level. The boundary is real; the primitive to express it is not.
...
3. **Resource collision is namespace-dependent.** Two modules that both define a `db` resource collide. Developers avoid this by namespacing (`github.client`, `linear.client`), but the framework doesn't enforce or validate the boundary. Packs make the namespace real.
...
`pack()` is a definition function, like `trail()`, `resource()`, and `signal()`. It returns a Pack. The topo accepts Packs alongside bare trail modules.
...
Resources within a pack are namespaced by the pack. The pack name provides the namespace boundary:

```typescript
// In github.core pack
const githubClient = resource('github.client', { /* ... */ });

// In linear.core pack
const linearClient = resource('linear.client', { /* ... */ });
```

The namespacing convention is already established (developers namespace by hand today). Packs formalize it. The warden can warn when a resource ID doesn't match its pack's namespace.
...
A pack's `requires` are resolved with mock resources during testing. The SDK wrapper's mock factory provides a mock GitHub client. The domain pack gets the mock client without knowing it's mocked. `testExamples` on a pack validates the pack's own trails with its own mock resources and its required packs' mocks.
...
$ trails survey --packs
Packs:
  github.core          4 resources, 8 trails (all internal)
  github.pull-requests 0 resources, 3 trails (3 public), requires: github.core
  firewatch.inbox      1 resource,  3 trails (3 public), requires: github.pull-requests
```
...
- **A new primitive.** `pack()` joins `trail()`, `resource()`, `signal()`, and `topo()` in the framework's definition vocabulary. Every new primitive is a concept to learn. The justification: the gap between trail and topo is real, and developers are already building ad-hoc boundaries with modules.
...
- **How packs are distributed.** npm packages, GitHub repos, scaffolded source, monorepo packages: all valid. The pack primitive is distribution-agnostic. The resources ADR addresses distribution.
...
- [ADR-0030: Contours as First-Class Domain Objects](../0030-contours-as-first-class-domain-objects.md) -- entities define the domain objects that pack trails operate on
