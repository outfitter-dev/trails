...
WebSocket breaks this model because it's two things over one persistent connection: request-response (client calls trails, gets Results) and server-push (clients subscribe to signals, receiving delivery records as they happen). The first part follows the established trailhead pattern. The second part has no precedent in the current trailhead model.
...
SSE (Server-Sent Events) on the HTTP trailhead is a lighter-weight option for server-push, but it's unidirectional (server to client only). WebSocket provides bidirectional communication: the client can call trails AND receive signals over the same connection. For applications that need both, WebSocket is the right trailhead.
...
- Circular delivery buffers with sequence numbers enable replay on reconnect
- `lastSeenSeq` on reconnect is sufficient for most replay needs

These patterns inform the design but don't constrain it. The Trails version can be cleaner because the framework provides typed contracts, a future signal delivery layer can provide external delivery, and the permit model provides authentication.

## Decision

### WebSocket follows the trailhead pattern

WebSocket is a trailhead. It has a `trailhead()` one-liner, a `build*` escape hatch, and it renders the topo's trails and signals for its transport.
...
Clients subscribe to signal types:

```json
{ "type": "subscribe", "signals": ["booking.confirmed", "booking.cancelled"] }
```

The trailhead would validate the signal IDs against the topo and the connection's permit. Response:

```json
{ "type": "subscribed", "signals": ["booking.confirmed", "booking.cancelled"] }
```

Or partial subscription if some signals are unauthorized:

```json
{
  "type": "subscribed",
  "signals": ["booking.confirmed"],
  "denied": [{ "signalId": "booking.cancelled", "reason": "insufficient scope" }]
}
```
...
```json
{ "type": "unsubscribe", "signals": ["booking.cancelled"] }
```
...
The connection is authenticated. The client can call trails and subscribe to signals. The connection's permit determines what's accessible.
...
If a future signal delivery surface adds replay, the trailhead would read signal delivery records from sequence 39 onward. Delivery records would be buffered in a per-connection circular buffer (configurable size, default 1000 records). If the client's `lastSeenSeq` is too old (outside the buffer window), the trailhead would send a gap notification:

```json
{ "type": "replay_gap", "from": 38, "available_from": 142, "message": "Records 39-141 are no longer available" }
```

The client would decide how to handle the gap: refetch state via trail invocations, or accept the loss. The framework would not mandate a recovery strategy.

That future trailhead would maintain a cursor position per connection: the last sequence number delivered. On reconnect, the trailhead would read forward from that position. For typed signals, it would need a durable signal delivery log or equivalent tracking layer; ADR-0038 does not provide external delivery, exactly-once semantics, or durable replay. "Replay" in this draft means catching up a reconnected client by reading forward from its last known position. If the backing data sources are durable, replay could survive server restarts.

### Permit-scoped discovery and signal subscriptions
...
Response filtered by permit scopes. `booking.*` signals would require `booking:read` scope. Namespace-to-scope mapping would match trail discovery.

**Subscription filtering.** A client could only subscribe to signals their permit authorizes. Attempts to subscribe to unauthorized signals would be denied in the subscription response.
...
2. **Throttle.** Drop low-priority delivery records (lifecycle signals before authored signals). Notify the client:
...
| `replayBuffer` | `number` | `1000` | Max delivery records in replay buffer per connection |
...
The package depends on `@ontrails/core` and would need a future durable signal delivery layer for subscription delivery. Without that layer, the WebSocket trailhead would operate in request-response-only mode: trail invocation works, signal subscription is unavailable.
...
**With visibility.** Internal trails are not callable over WebSocket. The `discover` message only returns public trails. Signal subscription would respect visibility: signals fired by internal trails can still be deliverable when the signal contract itself is public.

**With packs.** A pack's public trails are callable over WebSocket. A pack's public signals would be subscribable once a delivery layer exists. The pack author would not configure anything WebSocket-specific. The trailhead would derive everything from the topo.

**With permit-gated discovery.** The connection's permit scopes trail visibility and signal subscription. An agent connected with `booking:read` scope would see booking read trails and could subscribe to booking signals. Write trails and admin signals would be invisible.

**With observability.** WebSocket connections would be observed: connection established, authentication result, subscriptions, disconnection, replay requests. Combined with signal delivery tracking from a future delivery layer, the tracing system could provide full observability of the WebSocket trailhead.

## Consequences

### Positive

- **Bidirectional communication over one connection.** Clients call trails (request-response) and receive signals (server-push) without managing two separate connections or polling.
- **Signal subscriptions use typed signals.** No separate pub/sub system.
  `ctx.fire(signal, payload)` in any trail would feed WebSocket subscribers
  through the delivery surface this draft defines. One fire, multiple consumers.
- **Replay on reconnect.** Per-connection cursor positions and sequence numbers could let clients catch up after brief disconnections when a durable delivery log exists. The trailhead would read forward from the client's last seen position. The client sends `lastSeenSeq`.
- **Permit-scoped everything.** Trail discovery, signal subscription, and invocation all respect the connection's permit. An agent sees exactly what it's authorized to use.
...
- **Replay query cost.** On reconnect, the trailhead reads forward from the client's last cursor position. For clients that reconnect after long gaps, this may involve reading a large number of records from the signal delivery log or tracking layer. The configurable replay window (default 1000 delivery records) caps how far back a reconnect can reach.
...
- **Horizontal scaling.** When multiple server instances run behind a load balancer, signal subscriptions must reach the right server. This requires a shared delivery backbone that a future signal delivery layer can plug into. The WebSocket trailhead doesn't solve distributed delivery.
- **SSE as a lighter alternative.** Server-Sent Events on the HTTP trailhead could provide signal subscriptions without the complexity of WebSocket connection management. SSE is unidirectional (server to client) but sufficient for many use cases. Whether to add SSE support to the HTTP trailhead is a separate decision.
