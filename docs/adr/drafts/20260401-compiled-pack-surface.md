...
Trails has three surfaces today: CLI, MCP, and HTTP. Each one takes the trail contract and projects it into a consumption context. CLI projects into process.argv and stdout. MCP projects into JSON-RPC tool definitions. HTTP projects into request/response with route derivation. The pattern is consistent: the trail is the product, the surface is the rendering.

But there's a consumption context that doesn't have a surface yet: plain TypeScript.

When a developer builds a set of trails, the only way to consume them is through a surface protocol. Another TypeScript project can't just `import { validate }` and call a function. It has to spin up an MCP client, make HTTP requests, or shell out to the CLI. That's friction that discourages reuse and creates an artificial boundary between "Trails projects" and "everything else."
...
The core principle says: if the information exists in the system, don't ask the developer to restate it. The information for a publishable library already exists in the pack graph. The framework should derive the library surface the same way it derives CLI commands and MCP tools.

### Why the pack is the right compilation unit

A trail is too granular for a library boundary. A single `validate` function doesn't form a useful package. The pack is the natural module: it owns the service graph, the event topology, the namespace, and the configuration surface. It's the unit of coherence.

Compiling at the pack level means services resolve once at construction (not per-call), internal composition via follows stays hidden behind the public API, and the pack's namespace maps directly to the library's API surface. This matches how every real library works: you instantiate a client, then call methods.
...
### The library surface is not "exporting internals"

A library is not a window into Trails. It is a surface projection for in-process consumption, the same way CLI is a surface projection for terminal consumption. The contract layer sits above implementations and below surfaces. The library surface sits alongside CLI, MCP, and HTTP as a peer, with the same derivation posture and the same `build*` wiring shape.

## Decision

### Part 1: Programmatic consumption is a first-class surface

The library surface compiles a pack (or topo) into a publishable npm package. Like CLI, MCP, and HTTP, it is a projection of the trail contract into a consumption context. The consumption context is "typed function calls in a TypeScript/JavaScript project."

The compile step walks the pack and emits:
...
The library surface follows the same derivation properties as every other surface (per ADR-008): pure, deterministic, explicit lookup tables, overridable. Trail IDs become method names. Intent informs JSDoc annotations. Examples become documentation. Error taxonomy maps to thrown Error subclasses instead of exit codes or HTTP status codes.

### Part 2: The runtime shape

#### Factory pattern (packs with services or config)

When a pack declares services or config, the compiled library exports a factory function. The factory projects pack-level dependencies into consumer-supplied inputs. Config schemas become configuration parameters. The exact projection shape for services is intentionally left flexible so the compiler can map service declarations into the most idiomatic constructor interface (concrete adapter instances, config-driven creation, or some mix).
...
- Service health and startup failures surface as thrown errors from the factory (consistent with how `blaze()` handles builder failures on other surfaces)
...
This makes the compiled library instance effectively a generated adapter runtime. That's okay. The consumer sees a client with typed methods and a disposal hook. Internally, it's a pack with surfaces stripped away and services wired through.
...
The principle: **the framework disappears, the contract survives.** The primary library surface does not require consumers to know Zod, Trails, or any framework concept. They get typed functions, typed errors, and lifecycle management. That's a complete library.
...
### Part 6: Error handling at the surface boundary

The library surface unwraps `Result` at the boundary, just as every surface translates Results into its native idiom:

| Surface | Result.ok mapping | Result.err mapping |
| --- | --- | --- |
...
Internally, the trail implementation still returns `Result`. The library surface adapter unwraps it. This is the same pattern as HTTP mapping `Result.err(NotFoundError)` to a 404 response.
...
Whether bundled (as a `./cli` subpath and `bin` entry) or published separately, the CLI companion is the same trails compiled to the CLI surface. Same validation, same errors, same behavior as the programmatic API. The consumer can `npx @acme/email-kit-cli validate --address test@foo.com` without installing the library.

Conventionally, libraries and their CLIs are maintained as separate codebases that inevitably drift. Both surfaces reading the same contract makes drift structurally impossible.
...
**The CLI companion is free.** Every Trails project can ship a CLI alongside the library. Same validation, same errors, same behavior. No separate Commander setup, no duplicated flag definitions, no maintenance burden. The two surfaces read the same contract.
...
**Testing flows through.** `testExamples(app)` already validates every trail against every example. The library surface is tested by the same examples. The warden already checks the contract. Library consumers get the same quality guarantees without a separate test suite.
...
**A CLI they didn't have to build.** If the library ships a CLI companion, the consumer can use it from scripts, CI, or the terminal. No installation required. Same contract, different surface.
...
- **Pack scope = API surface.** Which trails are public? The pack boundary answers this.
...
If a pack doesn't make sense as a library, that's feedback on the pack design, not on the library surface.
...
- **Event subscription semantics.** The current `event()` primitive defines payload schemas and provenance. Whether the compiled library exposes an `.on()` subscription interface depends on the runtime event model, which is being designed in a separate event ADR. This ADR does not promise event subscriptions at the library surface. Once the event ADR lands, this decision should be revisited to determine how (and whether) events project through the library surface as an additive capability
- Whether `depot` (the pack registry concept) plays a role in library discovery or distribution
- How service mocking works across the library boundary. The mock factory exists on the service definition, but whether it's re-exported for consumer testing is a separate question
- The exact JSON Schema format (single schema vs. per-trail schemas, OpenAPI vs. plain JSON Schema)
- Whether non-TypeScript compilation targets (Python stubs, Go interfaces) are feasible as future surfaces
