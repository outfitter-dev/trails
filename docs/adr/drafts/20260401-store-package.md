...
The hexagonal architecture places surfaces (CLI, MCP, HTTP) on the left and infrastructure (databases, caches, APIs) on the right. Surfaces have first-class framework support: `blaze()` derives a full CLI from the trail contract, `buildMcpTools()` derives MCP tool definitions, `buildHttpRoutes()` derives HTTP routes. The left side of the hexagon is solved.
...
2. **The trail contract is the single source of truth.** This is the core premise (ADR-000). Surfaces, tests, governance, and now persistence all project from the same authored schema.
...
A new package, `@ontrails/store`, provides the framework-agnostic store model. It follows the same two-level architecture as surfaces:
...
```typescript
// No separate service() call needed
...
This eliminates the ceremony of wrapping a database in a `service()` call. The store collapses the service definition, table definitions, and connection management into one declaration. Like `blaze()` collapsing `buildCliCommands` + `toCommander` + `program.parse()`.
...
The derived tables are accessible through the escape hatch. The developer writes raw queries using the adapter's native query builder (Drizzle, Kysely, etc.) with the same table objects the framework derived. Override what's wrong.
...
`testAll(app)` works with zero additional configuration. The mock store with fixtures is resolved automatically when the service context detects a test environment.
...
- **Migration is out of scope.** The store derives DDL for the current schema state. Schema evolution (ALTER TABLE, data migrations) is delegated to the adapter's migration tooling (e.g., drizzle-kit).
