...
The store connector creates the appropriate full-text index:
...
When both `fts` and `vector` are declared, hybrid mode becomes available. The connector implements Reciprocal Rank Fusion to combine results from both indexes.
...
The store connector maintains search indexes automatically as CRUD operations occur:
...
When search is declared on a store entity and the developer uses `crud()` (see [ADR-0032: `deriveTrail()` and Trail Factories](../0032-derivetrail-and-trail-factories.md)), a search trail is automatically included:
...
For safety, the connector sanitizes inputs to prevent SQL injection. The query string is always parameterized, never interpolated.
...
- [ADR-0030: Contours as First-Class Domain Objects](../0030-contours-as-first-class-domain-objects.md) -- entities provide the domain object schemas that search declarations annotate
- [ADR-0032: `deriveTrail()` and Trail Factories](../0032-derivetrail-and-trail-factories.md) -- the `crud()` factory that auto-generates search trails
