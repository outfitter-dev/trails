...
**Raw `bun:sqlite`.** Zero dependencies, maximum control. But the developer writes SQL strings, loses type-safe query building, and handles connection management manually. Good for a minimal adapter, but doesn't address the query builder escape hatch needed for complex operations.
...
Drizzle ORM and the relevant dialect driver (e.g., `drizzle-orm/sqlite-core`) are peer dependencies. The adapter does not re-export Drizzle. The developer installs Drizzle alongside the adapter, the same way Commander is installed alongside `@ontrails/cli`.
...
### 3. `store()` composes `deriveTable` with service lifecycle
...
The dialect abstraction lives in the adapter, not in `@ontrails/store` core. The store abstraction doesn't know about SQL dialects.
...
- **Drizzle is a peer dependency.** The developer must install Drizzle alongside `@ontrails/store`. This is the same pattern as Commander for CLI, but it's an additional dependency. Developers who want zero ORM dependencies would need a future raw `bun:sqlite` adapter.
- **The Zod-to-Drizzle mapping is a new surface.** If Drizzle changes its column builder API, the mapping needs to be updated. The mapping is small and the Drizzle API is stable, but it's a maintenance surface.
