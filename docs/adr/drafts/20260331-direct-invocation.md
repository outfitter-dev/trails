...
1. **Open a surface.** Set up Commander, run the CLI binary, pass flags. Or start the MCP server, connect a client, call the tool. Or start the HTTP server, curl the route. Each requires infrastructure: a bin entry, a `surface()` call, a running server. The developer wants to run one trail, not boot an application.
...
- [ADR-0027: Trail Visibility and Surface Filtering](../0027-visibility-and-filtering.md) -- `trails run` can invoke internal trails (it's programmatic, like `run()`)
