...
The returned object is a `Trail`. `topo('myapp', gitLog)` works. `ctx.compose('git.log', input)` works. `trailhead(app)` trailheads it on CLI, MCP, HTTP. `testExamples` tests it (using the mock in CI, using the real binary in integration).
...
- [ADR-0027: Trail Visibility and Surface Filtering](../0027-visibility-and-filtering.md) -- rigged SDK wrapper packs use `visibility: 'internal'`
- [ADR-0032: `deriveTrail()` and Trail Factories](../0032-derivetrail-and-trail-factories.md) -- `deriveTrail()` and `ingest()` factory that produces trails from external input shapes
