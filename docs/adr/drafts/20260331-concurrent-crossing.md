...
`ctx.cross()` is the composition primitive. One trail crosses another by ID, passing input and receiving a Result. The calling trail controls flow with normal TypeScript: branch on errors, pass results forward, compose sequentially. The crossing declaration is a flat array of trail IDs the trail might call. The warden verifies declarations match code.
...
**Fan-out.** Multiple independent crossings, all concurrent. Partial failure may be acceptable. The most common parallel pattern.

**Fan-out/fan-in.** Concurrent crossings, then a step that merges their results. The merge depends on all (or some) branches completing.

**Conditional branching.** Cross different trails based on runtime conditions. Only one path executes.

**Optional enrichment.** Cross a trail if it succeeds, continue without it if it fails. The crossing is declared but not required.
...
"Cross this trail" and "cross these trails" are the same verb. The plural form is inherently concurrent because if the developer wanted them sequential, they'd write three separate awaits. There's no ambiguity about what the array form means.
...
The single-crossing overload never sees the options argument. It doesn't need concurrency control because it's one trail.
...
The developer expresses optionality by how they handle the Result, the same way they do with sequential crossings today. No `optional: true` annotation. No framework concept of "optional crossing." The Result model is the optionality primitive.

### The crossing declaration stays flat
...
### `ctx.crossAll()` as a separate method

```typescript
const results = await ctx.crossAll([
  ['notify.email', { userId }],
  ['notify.sms', { userId }],
]);
```

This was the initial design. A new method specifically for parallel execution. Rejected because it adds vocabulary that isn't needed. "Cross all" is just "cross" with multiple targets. The array overload communicates the same thing without a new name. One method doing one conceptual thing (cross trails) is simpler than two methods for two modes of the same thing.

Adding `crossAll` also opens the door to `crossAny`, `crossRace`, `crossFirst`, and other combinators. Each is a new method on context, a new concept to learn, a new thing the warden must understand. The overload approach avoids this proliferation: `ctx.cross()` is the composition primitive, period.

### Structured crossing declaration with groups and annotations
...
### `before`/`after`/`concurrent` annotations on crossing declarations
...
Concurrency relationships are similarly expressed by the array overload: if two crossings appear in the same array, they're concurrent. If they're separate awaits, they're sequential. The code structure is the concurrency declaration.
...
```typescript
const results = await ctx.cross.concurrent([
  ['notify.email', { userId }],
  ['notify.sms', { userId }],
]);
...
### Weighted or prioritized crossings
