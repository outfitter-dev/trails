...
| Bounded memory trace sink | `@ontrails/observability` | `@ontrails/tracing`, CLI trace rendering, tests | `owned` | Observability owns `createMemorySink` and `DEFAULT_MEMORY_SINK_MAX_RECORDS`. Tracing keeps a compatibility wrapper over the observability sink without a second retained-record implementation. |
