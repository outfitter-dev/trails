...
Missing graph artifacts are hard failures for locked graph reads and warnings for source outline context. Follow the diagnostic's selected-app compile shape when you are allowed to refresh artifacts: from a configured workspace root, use `trails compile --app <id> --root-dir . --permit '{"id":"operator","scopes":["topo:write"]}'`; from an explicitly bounded standalone app, use `trails compile --module <app-module> --root-dir <app-root> --permit '{"id":"operator","scopes":["topo:write"]}'`. Do not silently fall back to stale graph assumptions. Use source search, qmd, or symbol tools when Wayfinder reports missing or stale artifacts, when the task needs implementation text that Topography does not derive, or when the current authority does not allow generating fresh artifacts. Compile writes the selected app-root `trails.lock`; it never creates an aggregate workspace-root lock. Treat generated locks as evidence and clean them up unless the branch intentionally owns them. Do not treat `.trails/` as disposable compile output; it is committed control.
