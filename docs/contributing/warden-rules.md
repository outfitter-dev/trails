...
The repo-local `no-retired-lexicon-terms` Oxlint rule is the source-file guard for retired lexicon terms. It reads retired terms from the `docs/lexicon.md` Reserved Terms table and checks source-owned symbol roles such as identifiers, local or `@ontrails/*` import paths, object keys, and literal member properties. It intentionally does not lint Markdown prose. Governed transitions instead keep prose, source, classifications, and historical mentions visible through Regrade's occurrence ledger; Warden consumes the transition registry and immutable Regrade history to reject missing provenance or reintroduced source symbols. Migration docs, changelogs, ADRs, and contrastive explanations stay classified instead of being blindly rewritten. `bun run regrade:audit` combines the latest applied vocabulary plans with the current governed policy, scans Markdown, and fails when a completion gate reopens. `bun run check` runs the repo-local Oxlint rules, `trails warden`, and that Regrade audit so the durable owners enforce the final contract without a parallel root vocabulary-audit map.
