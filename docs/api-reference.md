...
// WardenOptions includes scope.exclude root-relative globs for paths Warden should not govern
...
prepareRegradeRun({ root, classes, identity, ... }) // in-memory classified evaluation
validatePreparedRegradeRun(prepared, currentIdentity) // read-only freshness check
applyPreparedRegradeRun(prepared, currentIdentity) // freshness-checked mutation
prepareVocabularyRegradeRun({ root, plan, identity, ... })
applyPreparedVocabularyRegradeRun(prepared, currentIdentity)
...
regradeHistoryReceiptSchema, regradeReceiptPlanSchema, regradeFormJudgmentSchema
REGRADE_HISTORY_RECEIPT_SCHEMA_VERSION
literalRegradeTopo, literalRegradeTrail

// Compact receipt serialization, resolution, and content addressing
...
RegradeFormJudgment, RegradeHistoryReceipt, RegradeReceiptPlan, RegradeReceiptPlanProvenance
ResolvedRegradeHistoryReceipt, ResolvedRegradeHistoryReceiptRun
```

Regrade reports always include full aggregate counts, unknown class IDs, scan statistics, and skip reasons. Git-derived `nested-worktree`, `nested-repository`, and `submodule-boundary` skips come from the shared Source collector before authored scope is applied; the selected root itself remains directly scannable. `regrade audit` carries the same `skippedByReason` counts in each transition summary. Review entries carry structured review details — span, symbol, node kind, matched form, expected target, candidate replacement, signals, preserve cautions, and suggested validation — with a `judgment` field distinguishing unresolved occurrence judgment from completed preserve/rewrite verdicts, populated where the producing class can supply them. Report `entries` default to actionable rewrite and review outcomes; pass `includeEntries: 'all'` to include no-op and skip entries. The `scan` block summarizes matched, scanned, and skipped files, then groups matched files by extension and top-level path segment; vocabulary reports include occurrence counts in those buckets. Vocabulary regrade runs add a `run` block with the authored plan, observed form and occurrence ledger, and completion gate so CLI and MCP callers can see what was applied, modified, skipped, or deferred with an `open` count. Applied rewrites remain in the live run report occurrence ledger with an `applied` verdict so aggregate dispositions and scope-tier totals reconcile after source changes. Each occurrence also carries an `in-scope` or `policy-classified` tier, and the report groups those counts, `historical-by-policy` dispositions, and expected/touched/missing `teachingSurfaces`. `scope.exclude` remains the hard boundary for mechanical noise that is never scanned; `scope.policyClassified` keeps protected path globs scanned and counted while preventing default rewrites. Vocabulary plans may also author `fileRenames: [{ from, to }]`. Preview derives reference closure from that map and scope without writing; apply validates every move, performs all moves first, then makes one bounded reference pass against the final paths. Exact code string literals use the AST engine, prose paths use the vocabulary boundary rules, ambiguous or residual code contexts route to review, and policy-classified references remain unchanged. `run.report.fileRenames` persists old/new paths plus rewritten, preserved, deferred, skipped, and historical counts through CLI, MCP, and consolidated history. The Trails app exposes that lifecycle as Regrade plan commands: `trails regrade plan <from> <to>` writes an active plan under `.trails/regrade/<slug>.json`, `trails regrade plans` lists active, stale, and applied plans, `trails regrade check` proves the saved plan is fresh and gate-green, `trails regrade preview` reruns the plan without writing, and `trails regrade apply` consumes the plan and appends a run entry to the transition's consolidated append-only history file at `.trails/regrade/history/<transition>.json`. `@ontrails/regrade` exposes the canonical v3 `RegradeHistoryReceipt` contract for receipt persistence: each run embeds or content-addresses authored intent and classified form judgments, records exact Git blob transitions plus source, policy, lock, and detail evidence hashes, and carries compact completion facts without duplicating occurrence ledgers or reports. `serializeRegradeHistoryReceipt` emits deterministic bytes, and `resolveRegradeHistoryReceipt` fails closed on malformed paths, stale hashes, broken references, duplicate identities, or contradictory completion evidence without consulting a cache. Application surfaces must cut over their history writer before claiming their histories use this contract. History files are generated evidence — never hand-edit them. A transition is never truly closed — the lifecycle is cyclic between active and history: `trails regrade adjust <transition>` pulls a graduated transition back into an active plan built from the last recorded run's authored intent, preserving the transition's stable id so the re-run's apply appends to the same consolidated history instead of forking it; the graduated history file itself is left untouched by adjust. `trails regrade plan --dry-run` derives and prints the plan artifact without writing it; `trails regrade apply --dry-run` proves the apply path without mutating source or writing history. `trails regrade plan --expand` stages wide-net candidates in the plan's `expansion` section with kind, value, evidence, suggested classification, and status; pending candidates are review inventory and are not applied until adopted into the primary authored plan. Project `trails.config.*` files may provide `regrade.scope` defaults for `include`, `exclude`, and `extensions`; explicit CLI or MCP plan inputs override those defaults. A large regrade can be applied in scoped parts across a PR stack by re-planning the same transition with progressively scoped `--include`/`--exclude` and applying each part — every scoped apply appends its stamped run to the same consolidated history, with no separate coordinating artifact. Class-mode plans accept `--name` to give multi-class transitions a clean transition-name key for the plan and history files instead of a concatenated class-id filename.

Plan, check, preview, apply, and adjust completion results include a `lifecycle` block with total wall-clock duration and ordered completed phases. Human CLI output streams those phases to stderr. JSON and JSONL output keep stderr quiet so machine-readable output and structured error diagnostics remain intact; the lifecycle block carries the same facts instead. MCP returns that block unchanged and sends progress notifications when the request includes a progress token. These observational timings are command-result metadata only: saved plans and v3 history receipts never include them, so timing cannot change receipt hashes or regeneration.

Apply retains its preflight classification in memory and reuses it only after a fail-closed freshness check. The identity uses the receipt vocabulary for plan content, policy/registry, scope, lock state, and tool version, while a separate source-state hash covers exact root-relative paths and raw bytes; class runs also hash the nearest package manifests that provide classification context. Project Warden policy identity resolves the local import graph from each discoverable rule and hashes every project-owned module byte, including underscore-prefixed or outside-`.trails` support modules, so closure-captured rule changes cannot hide behind stable function source. Source additions, removals, edits, unreadable paths, plan edits, registry changes, scope or entry-detail changes, lock changes, and tool-version changes reject the prepared run. The handle is process-local and object-identity-bound: it is never serialized, persisted, or required to regenerate a receipt. Vocabulary apply deliberately reruns its symbol preview before mutation and reevaluates the symbol lane after a prepared prose mutation so offset-based rewrites and rollback snapshots observe the current tree; only the independently fresh prose lane is reused. The post-apply completion scan remains mandatory.

Current consolidated histories use the Regrade-owned v3 receipt contract. Each run records compact authored intent with exact field provenance, classified form judgments, completion facts, exact before/after Git blob identities, source and policy evidence keys, and a project-relative root. Unchanged intent or classified state is hash-referenced to an earlier run in the same file; resolution never depends on a cache. Intent hashes cover both the plan body and its provenance so `regrade adjust` restores authored decisions without promoting derived fields. The serializer emits canonical bytes, repository formatters exclude generated history, and readers fail closed on broken references, hash drift, non-normalized or absolute paths, and embedded/observed path mismatches. The tool selects the generator-owned filename, while `regrade adjust` and graduated `regrade check` select it by the opaque stable transition ID returned by apply. Warden validates an independent narrow projection of these receipts and consumes compact form judgments rather than persisted execution ledgers.

For vocabulary work, `trails regrade plan <from> <to>` treats that pair as a seed rather than a complete hand-authored migration. Its saved `derivation` section deterministically proposes safe and review-only morphology, source-observed compound and public-identifier review candidates, review-only filename moves with reference-closure evidence, namespace occurrence counts, and preserves for matching trail IDs and library exports found in `trails.lock`. Every item records `authored` or `derived` provenance. An incompatible lock fails planning rather than silently removing the live-API safety boundary. The observed run ledger remains separate execution evidence, and `--expand` remains an optional wider discovery pass; neither review inventory nor filename proposals apply until they are explicitly adopted into the plan.
