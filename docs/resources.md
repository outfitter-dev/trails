...
The config values come from the resolved app config, keyed by resource ID. `@ontrails/config` provides `collectProvisionConfigs()` to gather all resource config schemas from a topo, and `defineConfig()` to wire profile-based resolution into the bootstrap pipeline. Resources without a `config` schema receive `unknown` and ignore `svc.config`.
