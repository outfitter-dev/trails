...
4. Compose layers
5. Enter the implementation

This means failures surface at the boundary -- a missing `DATABASE_URL` fails before execution enters the implementation, not on line 47. It also means layers can access resources via `db.from(ctx)` because resolution is already complete.
