...
`release:none` is the explicit no-release disposition. Use it only when the branch touches package files but does not change user-visible package content. The PR body must include a rationale that mentions `release:none`; if the original rationale lives in an issue or handoff, mirror the sentence into the PR body. A good `release:none` rationale names the affected files and explains why users do not need a package changelog entry. A bad rationale merely says "internal" or "test only" while the branch also changes public trail additions/removals, visibility, input, output, surface exposure, generated artifacts, or package docs.
