...
The refreshed bundle is recorded as plugin manifest version `0.3.3`. If the marketplace requires another version bump before publication, update `plugin/.claude-plugin/plugin.json`, then run:
...
5. Confirm whether plugin version remains `0.3.3` or is bumped again, then rerun
   metadata checks.
