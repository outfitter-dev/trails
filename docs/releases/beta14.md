...
Historical note: this table records the beta14 vocabulary at the time. The current typed-signal API is `fires` plus `ctx.fire(signal, payload)`; do not use the beta14 `signals:` / `ctx.signal()` names in new code.
...
| `dispatch()` | `run()` |
| `adapter` | `connector` |
| `@ontrails/crumbs` | `@ontrails/tracker` |
