...
**Auth and permit model (`@ontrails/permits`).** The `permit` field on trail specs declares scope requirements. Surface-specific extraction resolves credentials, `AuthConnector` turns them into a `Permit` (identity, scopes, roles), and `executeTrail` enforces scope intrinsically before the implementation runs. Includes JWT connector, governance rules (`validatePermits`), and test helpers through `@ontrails/permits/testing`.
