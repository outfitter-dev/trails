...
### Repair Apps Generated by 0.2.0

The published `0.2.0` CLI omits a direct `@types/node` development dependency from generated apps. A cold Bun install can select older transitive types and then fail typechecking imports such as `node:fs` and `node:path`.

From the generated app directory (`my-app` for a standalone app, or `my-app/apps/my-app` for a workspace), run:

```bash
bun add --dev '@types/node@^25.5.0'
bun run typecheck
bun test
```
