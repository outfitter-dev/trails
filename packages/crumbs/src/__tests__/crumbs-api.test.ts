...
import { createCrumbsApi } from '../crumbs-api.js';
...
describe('createCrumbsApi', () => {
  describe('span()', () => {
    test('creates a child record in the sink', async () => {
      const sink = createMemorySink();
      const { api } = createCrumbsApi(makeCtx(), sink);
    ...
    test('skips sink writes when trace sampling is disabled', async () => {
      const sink = createMemorySink();
      const { api } = createCrumbsApi(makeCtx({ sampled: false }), sink);
  ...
  describe('annotate()', () => {
    ...
    test('getAnnotations returns merged attrs from annotate calls', () => {
      const sink = createMemorySink();
      const { api, getAnnotations } = createCrumbsApi(makeCtx(), sink);
    ...
    test('getAnnotations returns empty object when no annotations', () => {
      const sink = createMemorySink();
      const { getAnnotations } = createCrumbsApi(makeCtx(), sink);
