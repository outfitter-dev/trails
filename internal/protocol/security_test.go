...
func TestSecureCommand_Verify(t *testing.T) {
	...
	// Generate valid signature with canonical serialization
	mac := hmac.New(sha256.New, secret)
	canonical, _ := json.Marshal(cmd)
	mac.Write(canonical)
...
func TestRateLimiter_Concurrency(t *testing.T) {
	...
	// May or may not have denied requests depending on timing\n	assert.GreaterOrEqual(t, denied, 0)
