...
const workspaceReplacementFor = (specifier: string): string | undefined => {
  ...
  return TARGET_RANGE.test(range) ? range : replacementFor(range);
};
...
const planSpecifier = ({
  ...
  if (
    TARGET_RANGE.test(source) ||
    (localPackages.has(effectiveName) && source.startsWith('workspace:'))
  ) {
    return { effectiveName };
  }
  return { effectiveName, unsupported: true };
...
const containsTrailsDeclaration = (value: unknown): boolean =>
  typeof value === 'string'
    ? value.includes('@ontrails/')
    : isPlainObject(value) &&
      Object.entries(value).some(
        ([name, nested]) =>
          name.includes('@ontrails/') || containsTrailsDeclaration(nested)
      );
