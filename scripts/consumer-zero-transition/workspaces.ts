...
const discoverGlobWorkspaceRoutes = ({
  root,
  rootDocument,
}: {
  ...
  readonly routeUsesSymlink: boolean;
}[] =>
  listWorkspacePatterns(rootDocument.manifest).flatMap((pattern) => {
    ...
    return entries.flatMap((entry) => {
      ...
      const lexicalPackageRoot = resolve(parent, entry.name).replaceAll(
        '\\',
        '/'
      );
      let packageRoot: string;
      try {
        packageRoot = realpathSync(lexicalPackageRoot).replaceAll('\\', '/');
      } catch {
        return [];
      }
      ...
      return [
        {
          insideRoot: isWithinRoot(packageRoot, root),
          name: document.manifest['name'],
          packageRoot,
          routeUsesSymlink: lexicalPackageRoot !== packageRoot,
        },
      ];
    });
  });
...
export const discoverConsumerWorkspace = ({
  ...
  const externalWorkspacePackages = workspaces.filter(
    ({ packageRoot }) => !isWithinRoot(packageRoot, root)
  );
  const externalWorkspaces = externalWorkspacePackages.flatMap(
    ({ manifest, packageRoot }) =>
      typeof manifest.name === 'string' && publicPackages.has(manifest.name)
        ? [{ name: manifest.name, packageRoot }]
        : []
  );
  const unsupportedExternalWorkspaces = externalWorkspacePackages.flatMap(
    ({ manifest }) =>
      typeof manifest.name === 'string' && !publicPackages.has(manifest.name)
        ? [manifest.name]
        : []
  );
  const globWorkspaceRoutes = discoverGlobWorkspaceRoutes({
    root,
    rootDocument,
  });
  const externalGlobWorkspaces = globWorkspaceRoutes.filter(
    ({ insideRoot }) => !insideRoot
  );
  const internalSymlinkWorkspaces = globWorkspaceRoutes.filter(
    ({ insideRoot, routeUsesSymlink }) => insideRoot && routeUsesSymlink
  );
  const documents = [
    rootDocument,
    ...localWorkspaces
  ...
  return {
    ...
    diagnostics: [
      ...
      ...externalGlobWorkspaces.map(({ name }) => ({
        file: rootDocument.file,
        message: publicPackages.has(name)
          ? `External Trails workspace ${name} is reached through a glob or ambiguous workspace entry; detach it manually.`
          : `External workspace ${name} is outside the selected consumer root; migrate it separately or select a broader root before applying.`,
        name,
      })),
      ...unsupportedExternalWorkspaces.map((name) => ({
        file: rootDocument.file,
        message: `External workspace ${name} is outside the selected consumer root; migrate it separately or select a broader root before applying.`,
        name,
      })),
      ...internalSymlinkWorkspaces.map(({ name, packageRoot }) => ({
        file: rootDocument.file,
        message: `Workspace ${name} is reached through an in-consumer symlink matched by a glob at ${packageRoot}; use its exact real path in workspaces before migration.`,
        name,
      })),
