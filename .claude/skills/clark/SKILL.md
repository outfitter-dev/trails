...
Load the appropriate skill or reference based on the task. In Claude, load the named skill directly. In Codex, if a named skill is not available through the skill loader, read the matching `.agents/skills/<skill-name>/SKILL.md` symlink. For Clark's relative references, read them under `.agents/skills/clark/references/`.
